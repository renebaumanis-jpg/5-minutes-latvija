...
def welcome_html(subscriber_email: str) -> str:
    ...
    if unsub_base:
        ...
        email_b64 = base64.urlsafe_b64encode(subscriber_email.encode()).decode()
        sig = hmac.new(secret.encode(), subscriber_email.encode(), hashlib.sha256).hexdigest()[:16]
        token = f"{email_b64}.{sig}"
        unsub_url = f"{unsub_base}/unsubscribe?token={token}"
        unsub_line = (
            f'<a href="{unsub_url}" style="color:#9e1c20;text-decoration:none;'
            f'font-size:11px;letter-spacing:1.5px;text-transform:uppercase;font-weight:600">'
            f'Atrakstīties</a>'
        )
    else:
        unsub_line = ''
